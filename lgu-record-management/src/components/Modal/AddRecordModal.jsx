...
export default function AddRecordModal({ isOpen, onClose, onSubmit, isLoading }) {
  ...
  const validateFile = (file) => {
    ...
    if (!validTypes.includes(file.type)) {
      alert(`File "${file.name}" is not supported. Allowed: PDF, DOC, DOCX`);
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({ category, files, number, title, date, sponsor, committee, expiryDate });
  ...
  return (
    <div className="fixed inset-0 bg-transparent bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full border border-slate-200 flex flex-col max-h-[90vh]">
              ...
              {files.length > 0 && (
                <div className="space-y-2 max-h-40 overflow-y-auto mt-2">
                  {files.map((file, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-2 md:p-3 bg-slate-100 rounded-lg text-sm hover:bg-slate-200 transition"
                    >
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-slate-200 flex flex-col sm:flex-row gap-3 shrink-0 bg-gray-50 rounded-b-xl">
          <button
            onClick={onClose}
            type="button"
          ...
          <button
            ...
            className="flex-1 px-4 py-2.5 bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-700 transition-colors flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
